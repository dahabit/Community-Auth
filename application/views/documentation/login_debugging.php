...
<p>
	You will first need to open up <b>/application/config/config.php</b>. Find the log_threshold setting and change it to 2 or greater than 2. 2 is the lowest level that allows for debug level error messages to be logged.
</p>
<div class="doc_code">
...
<h3>Now what?</h3>
<p>
	First, unless you changed the name of the form elements in the login form, the string, password, and form_token values should always be set. If they aren't then you're going to need to revert to the original names.
</p>
<p>
	A more likely problem to see here is that the form_token doesn't match the flash_token. If this is the case, you need to look for 404 errors in your server access logs (not the CI log file). Your server access logs could point to a 404 for an image or some other asset that doesn't exist. Since a 404 error "uses up" the flash token, you'll never be able to login until you fix that. Also, every once-in-a-while, I've found that FirePHP being on won't allow me to login or stay logged in. If your form_token doesn't match your flash_token and you're using FirePHP, turn off FirePHP and try again.
</p>
